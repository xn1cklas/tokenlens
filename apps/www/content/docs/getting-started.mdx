---
title: "Getting Started with Tokenlens"
description: "Install Tokenlens, use quick helpers, and configure a custom instance."
---

## 1. Installation

```bash
npm install tokenlens
# or
yarn add tokenlens
# or
pnpm add tokenlens
```

Tokenlens ships TypeScript types out of the box. Node.js 20+ (or a Fetch-compatible runtime) is required for the hosted catalogs (OpenRouter default, models.dev, Vercel, etc.).

Token counting helpers (`countTokens` and `estimateCostUSD`) can load `@tokenlens/tokenizer` on demand. Install it alongside `tokenlens` when you want Tokenlens-managed token counting, or inject your own tokenizer when your app already owns that logic:

```bash
npm install tokenlens @tokenlens/tokenizer
```

## 2. Quick usage (standalone helpers)

If you just need metadata, cost, or context helpers, import the top-level functions. They reuse a lazily-created Tokenlens instance configured with the default `catalog: "auto"` setting (an alias for the OpenRouter catalog). To use a different dataset, pass `{ catalog: "models.dev" }` to the helper or instantiate your own client with `createTokenlens({ catalog: "models.dev" })`. For custom catalogs, cache adapters, fetchers, overrides, or tokenizers, create a configured instance and pass it as `{ tokenlens }` to keep reuse explicit.

```ts
import {
  getModelData,
  listModels,
  computeCostUSD,
  getContextLimits,
} from "tokenlens";

const usage = { inputTokens: 2_000, outputTokens: 500 };

const details = await getModelData({ modelId: "openai/gpt-4o-mini" });
const costs = await computeCostUSD({ modelId: "openai/gpt-4o-mini", usage });
const limits = await getContextLimits({ modelId: "openai/gpt-4o-mini" });
const openaiModels = await listModels({ provider: "openai", search: "gpt-4o" });

console.log(details.id);              // "openai/gpt-4o-mini"
console.log(details.limit?.context);  // combined token limit
console.log(costs.totalTokenCostUSD); // USD estimate for provided usage
console.log(limits?.context);         // context window in tokens
console.log(openaiModels.length);     // matching models in the active catalog
```

## 3. Create a configured instance

Use `createTokenlens` when you need to pin a specific catalog, adjust caching, supply fixture data, load your own registry, or inject token counting:

```ts
import {
  createTokenlens,
  type TokenlensOptions,
  type CatalogOverrides,
  type SourceProviders,
} from "tokenlens";

const options: TokenlensOptions = {
  catalog: "openrouter",             // choose a built-in catalog
  ttlMs: 10 * 60 * 1000,             // refresh provider data every 10 minutes
  cacheKey: "tokenlens:openrouter",  // optional custom cache key
  timeoutMs: 3_000,                  // fail slow catalog requests
};

const tokenlens = createTokenlens(options);

const cost = await tokenlens.computeCostUSD({
  modelId: "openai/gpt-4o-mini",
  usage: { input_tokens: 400, output_tokens: 200 },
});

console.log(cost.totalTokenCostUSD);

// Provide your own catalog data (useful during tests)
const fixtureCatalog: SourceProviders = {
  demo: {
    id: "demo",
    models: {
      "demo/chat": {
        id: "demo/chat",
        canonical_id: "demo/chat",
        name: "Chat Demo",
        cost: { input: 1, output: 1 },
        limit: { context: 128_000, output: 4_096 },
      },
    },
  },
};

const fixtureTokenlens = createTokenlens({
  catalog: fixtureCatalog,
});

const sourceTokenlens = createTokenlens({
  catalog: {
    id: "private-registry",
    async load({ fetch = globalThis.fetch, signal } = {}) {
      const response = await fetch("https://example.com/models.json", {
        signal,
      });
      return response.json();
    },
  },
  tokenizer: ({ data }) => data.trim().split(/\s+/).filter(Boolean).length,
});

const priceOverrides: CatalogOverrides = {
  demo: {
    models: {
      "demo/chat": {
        cost: { input: 2 },
      },
    },
  },
};

const tokenlensWithOverrides = createTokenlens({
  catalog: "openrouter",
  overrides: priceOverrides,
});
```

### Controlling caching
- **TTL**: Use `ttlMs` to set how long provider catalogs should live in the cache.
- **Cache adapters**: supply `cache` with your own `{ get, set, delete }` implementation to back the cache with Redis or another storage.
- **No caching**: set `cache: false` to force a reload on every call and skip stale fallback (useful for tests or framework-owned caches).

## 4. Standalone vs. custom instance

| Use case                                          | Recommended approach                                                                |
| ------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Default catalog and minimal setup                 | Call the top-level helpers (`computeCostUSD`, `getModelData`, `getContextLimits`).  |
| Specific catalog or custom TTL/cache              | Create your own instance via `createTokenlens` and reuse it within your app.        |
| Fixture or offline catalogs                       | Pass a `SourceProviders` object via `catalog` when creating the instance.           |
| Generated or private registries                   | Pass a `CatalogSource` object via `catalog` and configure cache/fetch controls.     |
| Existing app-owned token counting                 | Pass a `tokenizer` function when creating the instance.                             |
| Custom prices or local model corrections          | Use `overrides` with a hosted or custom base catalog.                               |
| Listing or searching catalog data                 | Use `listProviders`, `listModels`, or `tryGetModelData`.                            |

## 5. TypeScript types

Tokenlens re-exports the primary DTOs so you can import them from the root package:

```ts
import type {
  Usage,
  ModelDetails,
  TokenCosts,
  TokenlensOptions,
  CatalogOverrides,
} from "tokenlens";

const details = await getModelData({
  modelId: "openai/gpt-4o-mini",
});

const costs = await computeCostUSD({
  modelId: "openai/gpt-4o-mini",
  usage: { input_tokens: 1_000, output_tokens: 200 } satisfies Usage,
});
```

`ModelDetails` is simply the resolved `SourceModel` entry. Missing models throw `TokenlensError.ModelNotFound`. Use `computeCostUSD` and `getContextLimits` for derived information.

## 6. Next steps
- Configure advanced behaviour: [Sources & Caching](./sources-and-caching.md)
- Integrate with the Vercel AI SDK: [Integration Guide](./integrations/vercel-ai-sdk.md)
- Explore DTO terminology: [Glossary](./glossary.md)
- Migrating from v1: [Migration Guide](./migration-v1-to-v2.md)

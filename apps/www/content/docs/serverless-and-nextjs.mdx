...
export async function GET() {
  ...
  const details = await tokenlens.describeModel({ modelId: "openai/gpt-4o-mini" });
...
export const getModel = cache(async (modelId: string) => {
  return tokenlens.describeModel({ modelId });
});
```
...
For general caching options in Tokenlens (custom adapters, Redis, etc.), see [Sources & Caching](./sources-and-caching.md).

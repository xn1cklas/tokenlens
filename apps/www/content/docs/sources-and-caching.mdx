...
Use `CatalogSource` for registries you generate, host privately, or assemble from infrastructure at runtime. Tokenlens caches the loaded source the same way it caches hosted gateways.
...
Hosted gateways and async `CatalogSource` loaders receive the same fetch controls:

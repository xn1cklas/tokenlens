...
- Delete usages of registry globals (`MODEL_IDS`, `resolveModel`, etc.); use `listModels`, `listProviders`, or `tryGetModelData` for async discovery.
...
- **Custom instances**: `createTokenlens(options)` lets you choose a catalog id, `SourceProviders` object, or `CatalogSource`, tweak TTL, plug in your own cache adapter, disable caching with `cache: false`, control fetches with `signal`/`timeoutMs`, or inject a tokenizer.
- **Caching**: by default, provider catalogs are cached in-memory for 24 hours with jitter. Control this via `ttlMs` or a custom `cache`; use `cache: false` when a framework owns caching. Errors fall back to the last cached value when possible; set `staleIfError: false` when refresh failures should surface.
...
| Registry helpers     | Global listings (`MODEL_IDS`, `resolveModel`, sync `listModels`).                                                                | Use async discovery helpers: `tokenlens.listModels()`, `tokenlens.listProviders()`, and `tokenlens.tryGetModelData()`.                                                                               | Keeps v2 runtime lean without forcing consumers to flatten raw provider maps.               |
...
const matches = await listModels({ provider: "openai", search: "gpt-4o" });
```
...
**New**
```ts
const alts = await tokenlens.listModels({
  provider: "openai",
  search: "gpt-4o",
});
...
const allIds = (await tokenlens.listModels()).map((model) => model.id);
```
...
Pass a `SourceProviders` fixture via `createTokenlens({ catalog: fixtureProviders, cache: false })`.

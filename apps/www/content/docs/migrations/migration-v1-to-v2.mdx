...
- **Custom instances**: `createTokenlens(options)` lets you choose a catalog (gateway id, `SourceProviders`, or `CatalogSource`), tweak TTL, plug in your own cache adapter, control fetches with `signal`/`timeoutMs`, or inject a tokenizer.

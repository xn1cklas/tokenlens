...
- Swap static catalog imports for your own `Tokenlens` instance when you need a specific catalog or fixture data.
...
- Update docs/README snippets to use the new API names and call out the default OpenRouter catalog (set `catalog: "auto"` for automatic selection).

## The Tokenlens class at a glance
- **Single responsibility**: load provider data from a selected `catalog` (falls back to `openrouter`; pass `catalog: "auto"` to probe available gateways) and expose consistent helpers (`getModelData`, `computeCostUSD`, `getContextLimits`, `getContextHealth`, etc.).
- **Lazy shared instance**: module-level helpers create a default instance on first use; without options they use the `"auto"` gateway, so override with `{ gateway: "openrouter" }` (or another id) when needed.
...
// or configure: createTokenlens({ catalog: "auto" }) / createTokenlens({ catalog: "models.dev" })
```

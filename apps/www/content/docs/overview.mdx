...
Pass one of these identifiers via `catalog` to switch sources, or supply your own `SourceProviders` object to run entirely on local fixture data (useful for tests and offline tooling). Use `overrides` when you want hosted metadata plus local price or limit corrections.

...
## createTokenlens
- Convenience factory that instantiates `Tokenlens` with sensible defaults (OpenRouter catalog when none provided; pass `{ catalog: "auto" }` for automatic selection).

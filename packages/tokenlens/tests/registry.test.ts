...
describe('raw accessors and type guards', () => {
  it('getModelRaw only returns canonical ids, not aliases', () => {
    const canonical = MODEL_IDS.find((id) => (models as Record<string, unknown>)[id]);
    expect(typeof canonical).toBe('string');
    const idx = (canonical as string).indexOf(':');
    const short = idx > 0 ? (canonical as string).slice(idx + 1) : (canonical as string);
  ...
  it('isModelId and assertModelId behavior', () => {
    ...
    expect(() => assertModelId('definitely-not-a-model')).toThrow(/Unknown model id/);
  });
});

...
/** Single model entry from models.dev */
...
/** Provider entry keyed at the top-level of the API JSON */
...
/** The full models.dev API JSON shape */
...
export class FetchModelsError extends Error {
    ...
    constructor(opts: { code: FetchModelsError['code']; message: string; status?: number }) {
        super(opts.message);
...
export async function fetchModels(opts: { provider?: undefined; model?: undefined; fetch?: FetchLike; signal?: unknown; baseUrl?: string }): Promise<ModelsDevApi>;
export async function fetchModels(opts: { provider: string; model?: undefined; fetch?: FetchLike; signal?: unknown; baseUrl?: string }): Promise<ModelsDevProvider | undefined>;
export async function fetchModels(opts: { provider?: undefined; model: string; fetch?: FetchLike; signal?: unknown; baseUrl?: string }): Promise<Array<{ provider: string; model: ModelsDevModel }>>;
export async function fetchModels(opts: { provider: string; model: string; fetch?: FetchLike; signal?: unknown; baseUrl?: string }): Promise<ModelsDevModel | undefined>;
...
export async function fetchModels(opts: FetchModelsOptions = {}): Promise<
    ModelsDevApi | ModelsDevProvider | ModelsDevModel | Array<{ provider: string; model: ModelsDevModel }> | undefined
> {
    const url = opts.baseUrl ?? 'https://models.dev/api.json';
    const fetchImpl: FetchLike | undefined = opts.fetch ?? (globalThis as { fetch?: FetchLike }).fetch;
    ...
    if (!res.ok) {
        ...
        const snippet = body ? ` Body: ${body.slice(0, 256)}${body.length > 256 ? '…' : ''}` : '';
        throw new FetchModelsError({
            code: 'HTTP',
            status: res.status,
    ...
    // Runtime shape guard (lightweight)

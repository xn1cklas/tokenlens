  ...
  const items = models
    .map((m) => {
      const parts = [
        ...
        m.displayName ? `    displayName: ${JSON.stringify(m.displayName)},` : undefined,
        m.family ? `    family: ${JSON.stringify(m.family)},` : undefined,
        `    status: ${JSON.stringify(m.status)},`,
        `    context: ${JSON.stringify(m.context)},`,
        m.modalities ? `    modalities: ${JSON.stringify(m.modalities)},` : undefined,
        m.pricing ? `    pricing: ${JSON.stringify(m.pricing)},` : undefined,
        m.pricingSource ? `    pricingSource: ${JSON.stringify(m.pricingSource)},` : undefined,
        m.aliases && m.aliases.length ? `    aliases: ${JSON.stringify(m.aliases)},` : undefined,
        m.releasedAt ? `    releasedAt: ${JSON.stringify(m.releasedAt)},` : undefined,
        `    source: ${JSON.stringify(m.source)},`,
        m.contextSource ? `    contextSource: ${JSON.stringify(m.contextSource)},` : undefined,
        m.verifiedAt ? `    verifiedAt: ${JSON.stringify(m.verifiedAt)},` : undefined,
        `  }`,
      ].filter(Boolean);
      return parts.join('\n');
    })
...
function mapFromModelsDev(json) {
  ...
  for (const [provider, p] of Object.entries(json)) {
    ...
    for (const [modelId, m] of Object.entries(models)) {
      const inputMods = Array.isArray(m.modalities?.input) ? m.modalities.input : [];
      const outputMods = Array.isArray(m.modalities?.output) ? m.modalities.output : [];
      ...
      const mapped = {
        ...
        context: Object.fromEntries(
          Object.entries({ combinedMax, outputMax }).filter(([, v]) => typeof v === 'number')
        ),
        modalities:
          textIn || textOut || imageIn
            ? { ...(textIn && { textIn: true }), ...(textOut && { textOut: true }), ...(imageIn && { imageIn: true }) }
            : undefined,
        pricing:
          m.cost && (typeof m.cost.input === 'number' || typeof m.cost.output === 'number')
            ? {
                ...(typeof m.cost.input === 'number' ? { inputPerMTokens: m.cost.input } : {}),
                ...(typeof m.cost.output === 'number' ? { outputPerMTokens: m.cost.output } : {}),
              }
            : undefined,
  ...
  out.sort((a, b) => (a.provider === b.provider ? a.id.localeCompare(b.id) : a.provider.localeCompare(b.provider)));
  return out;
}
...
async function main() {
  ...
  console.log(`Wrote ${mapped.length} models to ${path.relative(path.join(__dirname, '..'), target)}`);
}
